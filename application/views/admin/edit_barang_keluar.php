<div class="container-fluid">
  <div class="row page-titles">
      <div class="col-md-5 align-self-center">
          <h4 class="text-themecolor">Ubah Barang Keluar</h4>
      </div>
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header" style="background-color: #d63b70">
          <h4 class="m-b-0 text-white">Form Barang Keluar</h4>
        </div>
        <div class="card-body">
    ...
    $.ajax({
      ...
      success: function(response){
        $.each(response.data, function(k, v){
          ...
          $('#ekspedisi').val(v.ekspedisi)
          $('#no_truk').val(v.no_truk)
          $('#ref_id').val(v.ref_id)
          $('#no_pesanan').val(v.no_sp)
    ...
    $('#form_edit').on('submit', function(e){
      ...
      if(id_customer === '' || alamat_kirim === '' || ekspedisi === '' || no_truk === '' || ref_id === ''){
        ...
        if(detail<1) {
          ...
          $.ajax({
            url: `<?= base_url('api/barang_keluar/edit/') ?>${auth.token}?no_keluar=${no_keluar}`,
            type: 'POST',
            dataType: 'JSON',
            beforeSend: function(){
