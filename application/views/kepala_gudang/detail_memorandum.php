...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        $.each(response.data, function(k, v){
          ...
          var btn = ''
          if(v.status === 'Proses'){
            btn+= `<button type="submit" id="btn_approve" data-id="${v.no_memo}" class="btn btn-success" style="margin-right: 10px; margin-bottom: 15px;">Approve<i class="ti-check"></i></button>`
    ...
    $('#action').on('click', '#btn_approve', function(){
      ...
      Swal.fire({
        title: `Apakah Anda yakin ingin mengapprove ${no_memo}?`,
        type: 'question',
        ...
        showLoaderOnConfirm: true
          ...
          if (result.value) {
            $.ajax({
              url: `<?= base_url('api/memorandum/approve/'); ?>${auth.token}?no_memo=${no_memo}`,
              type: 'GET',
              dataType: 'JSON',
              success: function(response){
